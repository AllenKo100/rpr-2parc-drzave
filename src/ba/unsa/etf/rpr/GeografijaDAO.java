...
public class GeografijaDAO {
    ...
    private Drzava dajDrzavuIzResultSeta(ResultSet rs, Grad grad) throws SQLException {
        int tip = rs.getInt(4);
        System.out.println("Drzava " + rs.getString(2) + " tip "+tip);
        if (tip == 2) {
            System.out.println("Tip je 2, vraćam Kraljevinu");
            return new Kraljevina(rs.getInt(1), rs.getString(2), grad);
        }
        else if (tip == 3)
            return new Republika(rs.getInt(1), rs.getString(2), grad);
        else
    ...
    public void dodajDrzavu(Drzava drzava) {
        try {
            ...
            dodajDrzavuUpit.setString(2, drzava.getNazivReal());
            dodajDrzavuUpit.setInt(3, drzava.getGlavniGrad().getId());
            dodajDrzavuUpit.setInt(4, tip);
    ...
    public void izmijeniDrzavu(Drzava drzava) {
        try {
            ...
            System.out.println("Postavljam naziv države na "+drzava.getNazivReal());
            promijeniDrzavuUpit.setString(1, drzava.getNazivReal());
